...
class BlockIndex(object):
    def __init__(self, r, c):
        assert( isinstance(r, (int, np.int64)) )
        assert( isinstance(c, (int, np.int64)) )
...
class Block(object):
    def __init__(self, x = 0, y = 0, h = 1, w = 1):
        if ( ( not isinstance(x, (int, np.int64)) ) or \
             ( not isinstance(y, (int, np.int64)) ) or \
             ( not isinstance(h, (int, np.int64)) ) or \
             ( not isinstance(w, (int, np.int64)) ) ):
    ...
    def set_size(self, h, w, flagUpdate = True):
        # Check the arguments.
        if ( ( not isinstance(h, (int, np.int64)) ) or ( not isinstance(w, (int, np.int64)) ) ):
...
class MisleadingBlock(Block):
    def __init__(self, x = 0, y = 0, h = 1, w = 1, value = 100, endPoint=None):
        super(MisleadingBlock, self).__init__(x, y, h, w)
    
        # Member variables defined in the super classes.
        self.color = "#FFFF00"
        self.name  = "MisleadingBlock"
        self.value = value
...
class GridMap2D(object):
    ...
    def __init__(self, rows, cols, origin = [0, 0], stepSize = [1, 1], name = "Default Name", outOfBoundValue = -10):
        assert( isinstance(rows, (int, np.int64)) )
        assert( isinstance(cols, (int, np.int64)) )
        assert( isinstance(origin[0], (int, np.int64)) )
        assert( isinstance(origin[1], (int, np.int64)) )
        assert( rows > 0 )
        assert( cols > 0 )
    ...
    def set_value_misleading_block(self, val):
        self.valueMisleadingBlock = val        
    ...
    def dump_JSON(self, fn):
        ...
        d = { \
            ...
            "valueMisleadingBlock": self.valueMisleadingBlock, \
    ...
    def read_JSON(self, fn):
        ...
        self.valueMisleadingBlock = d["valueMisleadingBlock"]
    ...
    def is_misleading_block(self, index):
        b = self.get_block(index)

        return isinstance( b, MisleadingBlock )        
    ...
    def get_index_misleading_block(self):
        """Return a copy of the index of the misleading block."""
    ...
    def is_in_misleading_block(self, coor):
        """Return ture if coor is in the misleading block."""
        ...
        idx = loc[3]
        if ( True == isinstance( self.blockRows[ idx.r ][ idx.c ], MisleadingBlock ) ):
            return True
    ...
    def is_around_misleading_block(self, coor, radius):
        """Return ture if coor is in a circle defined by the center of the misleading block."""
    ...
    def set_starting_block_s(self, r, c, value = None, startingPoint=None):
        ...
        if ( True == self.is_misleading_block( BlockIndex(r,c) ) ):
            raise GridMapException("The target index for starting block (%d, %d) is already assigned to a misleading block." % (r, c))        
    ...
    def set_misleading_block_s(self, r, c, value=None, endPoint=None):
        """
        endPoint is a two element list containing the misleading point coordinates.
        """
        assert( isinstance(r, (int, np.int64)) )
        assert( isinstance(c, (int, np.int64)) )
        ...
        if ( value is not None ):
            self.valueMisleadingBlock = value
    ...
    def set_misleading_block(self, index, value=None, endPoint=None):
        if ( isinstance( index, BlockIndex ) ):
            if ( endPoint is None ):
                self.set_misleading_block_s( index.r, index.c, value )
            else:
                self.set_misleading_block_s( index.r, index.c, value, endPoint=[ endPoint.x, endPoint.y ] )
        elif ( isinstance( index, (list, tuple) ) ):
            self.set_misleading_block_s( index[GridMap2D.I_R], index[GridMap2D.I_C], value, endPoint=endPoint )
        else:
            raise TypeError("index should be an object of BlockIndex or a list or a tuple.")        
    ...
    def random_misleading_block(self, value):
        raise NotImplementedError('Copy from random_ending_block and adjust')
...
% (self.rows, self.cols, self.origin[GridMap2D.I_X], self.origin[GridMap2D.I_Y], self.stepSize[GridMap2D.I_X], self.stepSize[GridMap2D.I_Y])
        ...
        # Get the string for misleading block.
        strMisleadingBlock = self.get_string_misleading_block()        
        ...
        s = "%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n" \
            % ( title, strDimensions, strNormalBlock, strStartingBlock, strMisleadingBlock, strEndingBlock, strObstacles, strCorners )
...
class GridMapEnv(object):
    ...
    def check_misleading_point_radius(self):
        """
        Check if the starting point's center is in the range of the ending point.
        """
        ...
        if ( False == self.map.haveMisleadingBlock ):
            raise GridMapException("Map of the environment does not have a misleading block.")
    ...
    def set_max_steps(self, m):
        assert( isinstance( m, (int, np.int64) ) )
        assert( m >= 0 )
    ...
    def enable_force_pause(self, t):
        ...
        assert( isinstance( t, (int, np.int64) ) )
        assert( t > 0 )
    ...
    def try_move(self, coorOri, coorDelta):
        ...
        if ( True == self.can_move( coor.x, coor.y, delta.dx, delta.dy ) ):
            ...
            if ( False == self.map.is_in_ending_block(coor) and False == self.map.is_in_misleading_block(coor)):
